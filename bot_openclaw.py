import feedparser
# import tweepy  # Twitter API 비활성화
import requests
import os
import sys
import time
...
GEMINI_API_KEY = get_clean_env("GEMINI_API_KEY") or "AIzaSyDSkxDygLSc_UHOGzkKmegx_63ktbEtUYc"
...
# ==========================================
# 2. 트위터 클라이언트 연결 (비활성화 - OpenClaw 사용)
# ==========================================
# Twitter API 연결 제거 - OpenClaw 브라우저 자동화로 대체
...
if __name__ == "__main__":
    ...
    for i, news in enumerate(selected_news):
        ...
        if img_path:
            try:
                # 이미지 파일을 twitter_queue/에 복사
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                new_img_name = f"news_{timestamp}_{i+1}.png"
                new_img_path = os.path.join(OUTPUT_DIR, new_img_name)
                shutil.copy(img_path, new_img_path)
                media_ids.append(new_img_path)  # 파일 경로 저장
                ...
                processed_count += 1
                if os.path.exists(img_path): os.remove(img_path)  # 원본 임시 파일 삭제
                print(f"  ✅ 이미지 저장: {new_img_path}")
            except Exception as e:
                print(f"  ❌ 파일 저장 실패: {e}")
    ...
    if media_ids:
        ...
        # JSON 파일로 저장 (OpenClaw가 읽어서 브라우저로 포스팅)
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            queue_file = os.path.join(OUTPUT_DIR, f"tweet_{timestamp}.json")
            queue_data = {
                "text": tweet_text_body,
                "images": media_ids,  # 파일 경로 리스트
                "created_at": timestamp
            }
            with open(queue_file, 'w', encoding='utf-8') as f:
                json.dump(queue_data, f, ensure_ascii=False, indent=2)
            
            print(f"🚀 [성공] 트윗 데이터 저장 완료: {queue_file}")
            print(f"  📝 텍스트 길이: {len(tweet_text_body)} 자")
            print(f"  🖼️ 이미지 개수: {len(media_ids)}")
            
        except Exception as e:
            print(f"❌ [실패] 파일 저장 에러: {e}")
    else:
